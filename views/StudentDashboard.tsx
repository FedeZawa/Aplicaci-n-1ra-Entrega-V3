            ...
            {MOCK_ANNOUNCEMENTS.filter((a: Announcement) => a.is_published).map((a: Announcement) => (
                <div key={a.id} className="bg-primary-500/10 border border-primary-500/20 p-4 rounded-xl flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-slate-300 text-sm">{a.content}</p> {/* content not message */}
                    </div>
                </div>
            ))}
                ...
                {selectedSession && (
                    <div className="space-y-6 text-center">
                        <div className="w-full h-32 rounded-xl overflow-hidden mb-4">
                            <img src={selectedSession.class?.image_url} className="w-full h-full object-cover" />
                        </div>
                        <div>
                            <h3 className="text-2xl font-bold text-white mb-1">{selectedSession.class?.name}</h3>
                            <p className="text-primary-500 font-medium">{selectedSession.class?.description}</p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            ...
                            <div className="bg-slate-950 p-3 rounded border border-slate-800">
                                ...
                                <span className="text-white">{selectedSession.class?.capacity! - (selectedSession.current_bookings_count || 0)} left</span>
                            </div>
                        </div>
            ...
            <div className="space-y-4">
                {history.length > 0 ? history.map((booking) => (
                    <Card key={booking.id} className="flex flex-col md:flex-row gap-4 items-center">
                        <img
                            src={booking.session?.class?.image_url}
                            className="w-full md:w-24 h-24 rounded-lg object-cover"
                            alt="Class"
                        />
                        <div className="flex-1 w-full text-center md:text-left">
                            <h3 className="font-bold text-lg text-white">{booking.session?.class?.name}</h3>
                            <p className="text-slate-400 text-sm">{booking.session?.class?.description}</p>
